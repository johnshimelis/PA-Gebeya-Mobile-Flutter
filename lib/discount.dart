...
class _DiscountScreenState extends State<DiscountScreen>
  ...
  final String _productsCacheKey = 'cached_discount_products';
  ...
  Future<List<Product>> _fetchProductsWithCacheFallback() async {
    try {
      ...
      if (cachedProducts != null && cachedProducts.isNotEmpty) {
        debugPrint("⚠️ Using cached discount products as fallback");
  ...
  Future<List<Product>> _fetchProductsFromNetwork() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint("🟢 Fetched ${data.length} discount products from network");
        return data.map((item) => Product.fromJson(item)).toList();
      } else {
        throw Exception(
            'Failed to load discount products: ${response.statusCode}');
      }
    } catch (e) {
  ...
  Future<List<Product>?> _getCachedProducts() async {
    try {
      ...
      debugPrint("❌ Error reading cached discount products: $e");
      return null;
    }
  }
  ...
  Future<void> _cacheProducts(List<Product> products) async {
    try {
      ...
      debugPrint("✅ Cached ${products.length} discount products");
    } catch (e) {
      debugPrint("❌ Error caching discount products: $e");
    }
  }
              ...
              if (snapshot.connectionState == ConnectionState.waiting) {
                ...
                return Center(
                  child: Column(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      const Text('No discounted products available'),
                      const SizedBox(height: 10),
                      ElevatedButton(
                        onPressed: _refreshProducts,
